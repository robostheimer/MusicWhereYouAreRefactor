...
{//
	...
	$favoriteArr = explode(']',$songartistArr[1]);
	$favorite = $favoriteArr[0];
	$location = $favoriteArr[1];	
	...
	$songsFinal=array('title'=>$songtitle[0], 'id'=> $songid, 'artist_name'=>$songartist, 'favorite'=>$artistlocation);
	
	//echo $songtitle[0].'-'.$songid[0].'-'.$songartist.'<br>';
		echo json_encode(array('title'=>$songtitle, 'id'=> $songid, 'artist_name'=>$songartist, 'favorite'=>$favorite, 'location'=>$location)).',';
	if($i==count($songsArr)-2)
	{
		echo json_encode(array('title'=>$songtitle, 'id'=> $songid[0], 'artist_name'=>$songartist, 'favorite'=>$favorite,'location'=>$location));	
	}
}
